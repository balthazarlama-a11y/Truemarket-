    ...
    try {
        ...
        if (method === "POST" && path === "/my/products") {
            ...
            const result = await getDb().insert(products).values({
                companyId: company.id, name: parsed.data.name,
                description: parsed.data.description || null, price: parsed.data.price || null,
                category: parsed.data.category || null, imageUrl: parsed.data.imageUrl || null, status: "active",
            }).returning();
            return res.status(201).json(result[0]);
        }
        ...
        if (method === "GET" && path === "/search") {
            ...
            const matchedProducts = await getDb()
                .select({ id: products.id, companyId: products.companyId, name: products.name, description: products.description, price: products.price, category: products.category, imageUrl: products.imageUrl, status: products.status, createdAt: products.createdAt, companyName: companies.companyName })
